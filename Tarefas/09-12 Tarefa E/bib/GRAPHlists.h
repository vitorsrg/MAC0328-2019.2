...
// Decide se o grafo não-dirigido G é aresta-biconexo. (Algoritmo
// quadrático: consome (V+E)*V unidades de tempo. Um pouco mais rápido
// que UGRAPHisEbc_v0().)
bool 
UGRAPHisEbc_v1( UGraph G);
 
// Função auxiliar que faz uma busca DFS no grafo G e calcula os vetores
// pre[], post[], pa[] e lo[] (lowest preorder number). Os quatro
// vetores são indexados pelos vértices de G. O vetor lo[] tem a
// seguinte propriedade: um arco x-y faz parte de uma ponte se e somente
// se x == pa[y] e lo[y] == pre[y]. 
void 
UGRAPHlo( UGraph G, int *pre, int *post, vertex *pa, int *lo);
...
// tempo. Código inspirado no programa 18.7, p.108, de Sedgewick.)
bool 
